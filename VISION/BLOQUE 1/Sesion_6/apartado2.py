...
img_original1 = imagen_original2 = img.copy()
...
for iteraciones in range(3): 
    ...
    erosionada = cv2.erode(img_original1,kernel,iteraciones)
    cv2.imwrite("Output/kernel%d.png" %(iteraciones+1) ,erosionada)
    imagen_original2 = erosionada
...
for iteraciones in range(3): 
    #fotogramas/frame%d.png" % cuenta ,concatenacion)
    erosionada = cv2.dilate(imagen_original2,kernel,iteraciones)
